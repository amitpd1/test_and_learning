def get_most_common_enemy(enemies_dict):
    #now check which enemy has highest count
    highest_count = 0
    highest_enemy_name = ""
    if(len(enemies_dict)) == 0:
        return None
    for key_name in enemies_dict:
        count_value = enemies_dict[key_name]
        if count_value > highest_count:
            highest_enemy_name = key_name
            highest_count = count_value
    return highest_enemy_name
